...
// Mobile-first responsive table components (client component with context)
import {
  CosmicTable,
  CosmicTableHeader,
  CosmicTableBody,
  CosmicTableRow,
  CosmicTableCell,
} from "@/components/shared/mdx/cosmic-table";

// Video player (client component, lazily loads react-player)
...
const components = {
  ...
  // ── Markdown native tables (plain MDX syntax) ────────────────────────────
  // These handle tables written with | --- | syntax in MDX.
  // For explicitly-composed tables, use the CosmicTable components below.

  table: ({ className, ...props }: React.HTMLAttributes<HTMLTableElement>) => (
    <div className="my-6 w-full overflow-x-auto rounded-lg border border-primary/20">
  ...
  tr: ({ className, ...props }: React.HTMLAttributes<HTMLTableRowElement>) => (
    <tr
      className={cn(
        "border-b border-primary/10 even:bg-muted/30 hover:bg-primary/5 transition-colors",
        className,
      )}
      {...props}
    />
  ...
  th: ({ className, ...props }: React.HTMLAttributes<HTMLTableCellElement>) => (
    <th
      className={cn(
        "px-4 py-3 text-left font-semibold text-primary text-xs uppercase tracking-wider bg-primary/10",
  ...
  td: ({ className, ...props }: React.HTMLAttributes<HTMLTableCellElement>) => (
    <td
      className={cn(
        "px-4 py-3 text-sm text-muted-foreground align-top",
        "[[align=center]]:text-center [[align=right]]:text-right",
        className,
      )}
      {...props}
  ...
  CosmicAlert: ({ children }: { children: React.ReactNode }) => (
    <Alert className="my-6 border-primary/50 bg-primary/10 relative overflow-hidden">
      ...
      <Sparkles className="h-4 w-4 text-primary relative z-10 shrink-0" />
  ...
  // ── Mobile-first CosmicTable (from cosmic-table.tsx) ─────────────────────
  // On mobile: each row renders as a labeled card.
  // On sm+: renders as a normal styled table.
  ...
  // ── Video Player ─────────────────────────────────────────────────────────
  // Lazily loads react-player. Accepts url, title, description, duration props.
